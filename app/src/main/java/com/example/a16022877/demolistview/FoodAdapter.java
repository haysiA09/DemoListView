package com.example.a16022877.demolistview;
...
public class FoodAdapter extends ArrayAdapter<Food> {
    ...
    public FoodAdapter(Context context, int resource, ArrayList<Food> objects) {
        ...
        food = objects;
        //Story Contect object as we would need to use it later
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        //The usual way to get the LayoutInflator object to "inflate" the XML file into a View object
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        //"Inflate" the row.xml as the layout for the View object
        View rowView = inflater.inflate(R.layout.row, parent, false);
        //Get the TextView obj
        tvFoodName = (TextView) rowView.findViewById(R.id.tvFoodName);
        ivStar = (ImageView) rowView.findViewById(R.id.ivStar);

        //The parameter "position is the index of the row LV is requesting
        //We get back the food at the same index
        Food currentFood = food.get(position);
        //set tv to show the food
        tvFoodName.setText(currentFood.getName());
        //Set the image too star or nostar accordingly
